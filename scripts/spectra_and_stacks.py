...
with open(main.MAGPI_sources, mode='r') as MAGPI_sources:
    ...
    for source in csv_reader:
        ...
        if field in magpiid[0:4] and main.z_min < redshift < main.z_max and QOP >= 3: # and main.z_min < redshift < main.z_max and QOP >= 3
            ...
            noise_indices = noise_indices = np.concatenate([np.arange(z_pixel - 40, z_pixel - 15), np.arange(z_pixel + 15, z_pixel + 40)])
            ...
            # if already detected, don't use binned spectrum



            if any(magpiid == row[0] for row in main.detections):
                plt.step(plot_bin_centres, plot_spectrum, where='mid', color='black', linewidth=1, label='Spectrum')

            else:
                plt.step(velocity_weakbin, binned_spectrum, where='mid', color='black', linewidth=1, label='Spectrum')
            ...
            # LASTLY, fit the guassian to the detection spectra
            if SNR > 3:
